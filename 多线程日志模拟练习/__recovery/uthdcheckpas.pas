...
begin
  while (not terminated) do begin
    ...
    if IsAnotherDay(NowDate) then begin
...
begin
  ...
  if FIsWarned then begin
    FIsWarned := false;
    if (AmtFree<START_DELETE_REMAINING_DISK_SPACE)  and (AmtFree>=0) then begin
  ...
  if (AmtFree < MINIMIUM_REMAINING_DISK_SPACE) and (AmtFree >= 0) then begin
